...
<style>
  ...
  .stButton > button, .stDownloadButton > button {{
    background: {PALETTE['persian_red']} !important;
    color: {PALETTE['white']} !important;
    border: 1px solid {PALETTE['persian_red']} !important;
    border-radius: 10px;
  }}
  .stButton > button:hover, .stDownloadButton > button:hover {{
    background: {PALETTE['verdigris']} !important;
    border-color: {PALETTE['verdigris']} !important;
  }}

  /* Inputs, selects, data editor, text inputs */
  div[data-baseweb="input"] > div, .stTextInput > div > div, .stNumberInput > div > div > input,
  ...
  /* Slider */
  .stSlider [role="slider"] {{ background: {PALETTE['persian_red']} !important; }}
  .stSlider [data-baseweb="slider"] > div > div {{
    background: {PALETTE['verdigris']}33 !important;
  }}
...
KCAL_PER_G_KEYS = ["energia (kcal/g)","energia kcal/g","calorias (kcal/g)","calorias kcal/g","kcal/g"]
KCAL_PER_100G_KEYS = ["energia (kcal/100g)","energia kcal/100g","calorias (kcal/100g)","calorias kcal/100g","kcal/100g"]
...
def nnls_iterative(A: np.ndarray, b: np.ndarray, max_iter: int = 50) -> np.ndarray:
    ...
    for _ in range(max_iter):
        ...
        keep = ~neg
        if keep.sum() == 0: return np.zeros_like(x)
        A_sub = A_s[:, keep]
        x_sub = np.maximum(0.0, np.linalg.lstsq(A_sub, b, rcond=None)[0])
        x = np.zeros_like(x); x[keep] = x_sub
...
def _tdee_by_method(day_label: str, adj_pct_value: float) -> float:
    ...
    if cal_mode == "Multiplicador":
        base = bmr_local * {"Alto": mult_alto, "Medio": mult_medio, "Bajo": mult_bajo}[day_label]
    else:
        base = bmr_local + {"Alto": float(extra_alto), "Medio": float(extra_medio), "Bajo": float(extra_bajo)}[day_label]
...
def carbs_g_per_kg_for_day(day_label: str, p_gkg: float, f_gkg: float, adj_pct_value: float) -> float:
    ...
    p_day_x = p_gkg * weight; f_day_x = f_gkg * weight
...
if cal_mode == "Multiplicador":
    base_tdee = bmr * {"Alto": mult_alto, "Medio": mult_medio, "Bajo": mult_bajo}[tipo_dia]
else:
    base_tdee = bmr + {"Alto": float(extra_alto), "Medio": float(extra_medio), "Bajo": float(extra_bajo)}[tipo_dia]
...
try:
    pie = (
        ...
        .mark_arc(stroke=PALETTE["raisin_black"], strokeWidth=1)
        .encode(
            ...
            color=alt.Color(
                ...
                scale=alt.Scale(
                    ...
                    range=[PALETTE["verdigris"], PALETTE["persian_red"], PALETTE["vanilla"]],
                ),
                legend=None
            ),
...
with st.expander("Editar reparto (proporción del día)"):
    ...
    warn_msgs = [f"suma {k} = {v:.2f}" for k, v in totals.items() if not (0.95 <= v <= 1.05)]
    if warn_msgs:
...
def meal_targets(meal_name: str, perc: Dict[str, float]) -> Dict[str, float]:
    p_t = p_day * perc["prot"]; f_t = f_day * perc["fat"]; c_t = c_day * perc["carb"]
    kcal_t = c_t*4 + p_t*4 + f_t*9
    return {"Comida": meal_name, "kcal": round(kcal_t,0),
            "Carbohidratos (g)": round(c_t,1), "Proteína (g)": round(p_t,1), "Grasa (g)": round(f_t,1)}
...
p_target = p_day * perc["prot"]; f_target = f_day * perc["fat"]; c_target = c_day * perc["carb"]
...
if foods.empty:
    ...
    if not selected.empty:
        ...
        editor_df = st.data_editor(
            editor_df, key=editor_key + "_table", use_container_width=True, hide_index=True,
            column_config={
        ...
        with btn_col1:
            if st.button("Ajustar TODOS (cuadrar macros)"):
                ...
                if len(unlocked_idx) == 0:
                    ...
                    if len(locked_idx) > 0:
                        A_lock = A_full[:, locked_idx]; g_lock = grams_now[locked_idx]
                        b_res = b - A_lock @ g_lock
                    else:
                        b_res = b
                    A_un = A_full[:, unlocked_idx]
                    x_un = nnls_iterative(A_un, b_res, max_iter=50)
                    new_grams = grams_now.copy(); new_grams[unlocked_idx] = x_un
        ...
        with btn_col2:
            ing_choice = st.selectbox("Ingrediente a ajustar (solo este)", editor_df["Producto"].tolist(), key=f"single_sel_{meal}")
            if st.button("Ajustar SOLO este ingrediente"):
                ...
                if denom <= 0 or not np.isfinite(denom):
                    ...
                    current_g = float(editor_df.loc[editor_df["Producto"] == ing_choice, "Gramos (g)"].iloc[0])
                    ...
                    st.success(f"Gramos {msg} en '{ing_choice}' en {abs(g_delta):.0f} g (nuevo total: {new_val:.0f} g).")
        ...
        if st.button("Guardar receta"):
            ...
            r = {
                ...
                "resultado": {"kcal": float(totals["kcal_g"]), "carb": float(totals["carb_g"]), "prot": float(totals["prot_g"]), "fat": float(totals["fat_g"])},
                "ingredientes": [{"producto": editor_df.loc[i,"Producto"], "gramos": float(editor_df.loc[i,"Gramos (g)"])} for i in range(len(editor_df))],
            }
