    ...
    try:
        if uploaded is not None:
            ...
            if uploaded.name.lower().endswith(".csv"):
                return _normalize_food_df(pd.read_csv(BytesIO(data)))
...
uploaded = st.sidebar.file_uploader("Sube alimentos (Excel o CSV)", type=["xlsx", "csv"])
st.sidebar.caption("CSV esperado: Alimento,Kcal,P,G,CH (por 100g)")
...
if foods.empty:
    st.warning("Sube un Excel/CSV de alimentos (o deja 'alimentos_800_especificos.xlsx' en la carpeta).")
else:
